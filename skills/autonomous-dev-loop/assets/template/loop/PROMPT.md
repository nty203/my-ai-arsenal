...
1. `loop/runtime/RUN_CONTEXT.md`가 있으면 읽는다.
2. `docs/feedback/INBOX.md`에서 미처리 최우선 지시를 찾는다.
3. `docs/DESIGN.md`에서 불변 목적과 범위를 확인한다.
4. `docs/STATUS.md`에서 현재 상태, 장애, 다음 후보를 확인한다.
5. 저장소의 `AGENTS.md`, `CLAUDE.md` 등 적용 가능한 상위 지침을 확인한다.
6. `docs/wiki/index.md`에서 관련 지식의 위치를 탐색한다.
7. 프로젝트 매니페스트, 진입점, 테스트, 최근 변경을 필요한 범위만 읽는다.
...
1. INBOX의 미처리 사용자 지시
2. STATUS의 장애 또는 실패한 Quality Gate
3. STATUS의 명시된 다음 목표
4. DESIGN과 현재 구현 사이의 가장 큰 검증 가능한 간극
...
### G5. 사용자 품질
- UI가 있으면 실제 화면을 열고 대표 스크린샷을 저장한 뒤 그 이미지를 직접 확인한다. 핵심 흐름, 빈 상태, 오류 상태, 반응형을 점검하며 코드/테스트 PASS를 시각 품질 PASS로 대신하지 않는다.
...
### G8. VCS 건전성
- 먼저 Git/SVN을 감지하고 해당 VCS의 status/diff/whitespace 검사를 사용한다. 기존 SVN working copy 안에 Git을 새로 만들지 않는다.
- checkpoint/commit에는 이번 작업 파일만 포함한다.
- 기존의 관련 없는 수정·미추적 파일은 보존하고 포함하지 않는다.
...
- **같은 지적은 두 번까지만**: 동일한 결함·운영 실패·사용자 지적이 두 번째 발생하면 규칙으로 승격하고, 측정 가능하면 자동 검사로 만들어 세 번째 재발을 막는다.
...
9. 종료: 다음 바퀴가 실행할 단 하나의 목표를 남기고 세션을 끝낸다.
